...
# import rasterio # Keep for fiona/clipping potentially, or simplify clipping too
...
# Need GeoRectangle for type hints and passing to make_temp_vrt potentially
from osgeo_utils.auxiliary.rectangle import GeoRectangle
from osgeo_utils.auxiliary.util import GetOutputDriverFor, open_ds  # Keep open_ds
...
from typing import (
    ...
    List,
    Sequence,
)  # Add Sequence for type hint
import fiona  # Keep for reading clip geometry source
import shutil  # For safer removal of temp directory
...
if not logger.handlers:
    ...
    formatter.reserved_attrs = tuple(list(formatter.reserved_attrs) + ["job_id"])
...
def mosaic_rasters_gdalcalc_style(
    ...
    try:
        ...
        for i, path in enumerate(raster_paths):
            ds = open_ds(path, access_mode=gdal.OF_RASTER | gdal.OF_VERBOSE_ERROR)
            if ds is None:
                raise RuntimeError(f"Failed to open input raster: {path}")
            ...
            if gt is None or dims[0] == 0 or dims[1] == 0:
                logger.warning(
                    f"Skipping {path} due to missing GeoTransform or zero dimensions."
                )
                ds = None  # Close dataset
                continue  # Skip this problematic input

            logger.debug(
                f"Opened: {path} (Res: {gt[1]:.4g}, {gt[5]:.4g} | NDV: {nodata})"
            )
            ...
            for x_block in range(nx_blocks):
                ...
                for i, read_ds in enumerate(aligned_datasets):
                    ...
                    if source_nodata is not None:
                        is_nan_nodata = numpy.isnan(source_nodata)
                        valid_mask = (
                            (~numpy.isnan(in_block_data))
                            if is_nan_nodata
                            else (in_block_data != source_nodata)
                        )
        ...
        for opt in cog_options:  # Use the list defined within this function
            try:
                ...
                local_cog_options_dict[key.upper()] = (
                    value  # Handle cases where split fails or int conversion fails
                )
        ...
        if out_ds_overview is None:
            ...
            blockxsize = local_cog_options_dict.get("BLOCKXSIZE", 512)  # Get int value
            blockysize = local_cog_options_dict.get("BLOCKYSIZE", 512)  # Get int value
            ...
            try:
                ...
                with fiona.open(clip_geometry_path, "r") as clip_src:
                    ...
                    for feature in clip_src:
                        ...
                        if not geom_json_str or geom_json_str.lower() == "none":
                            continue

                        ogr_geom = ogr.CreateGeometryFromJson(geom_json_str)
                        if ogr_geom is None:
                            logger.warning(
                                f"Skipping invalid geometry in feature {feature.get('id', 'N/A')}"
                            )
                            continue
                ...
                mem_layer = mem_ds.CreateLayer(
                    "clip", srs=target_sr, geom_type=ogr.wkbMultiPolygon
                )  # Assume polygons
                ...
                warp_options = gdal.WarpOptions(
                    ...
                    targetAlignedPixels=True,  # Good practice
                    multithread=True,
                    warpOptions=["NUM_THREADS=ALL_CPUS"],
                    warpMemoryLimit=int(os.environ.get("GDAL_CACHEMAX", 1024))
                    * 1024
                    * 1024
                    * 0.5,  # Use 50% of GDAL Cache limit for warp
                    # Ensure correct CRS handling if needed (should be aligned now)
                    # srcSRS=target_crs_wkt if target_crs_wkt else None,
                ...
                os.replace(temp_clip_output, str(mosaic_output_path))
        ...
        for vrt_file in temp_vrt_files:
            for ext in ["", ".aux.xml"]:  # Remove VRT and common sidecar file
                try:
                    ...
                    logger.warning(f"Could not remove temporary VRT file {fpath}: {e}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Paths (local or cloud URI) to input rasters.",
    )
    parser.add_argument(
        "--mosaic-output-path",
        type=Path,
    ...
    # Convert Paths to strings for GDAL/subprocess
    raster_paths_str = [str(p) for p in args.raster_paths]
    ...
    # Simplify COG options access
    cog_options_dict = dict(
        opt.split("=") for opt in ["TILED=YES", "BLOCKXSIZE=512", "BLOCKYSIZE=512"]
    )

    logger.info(f"Starting {JOB_ID} job (gdal_calc style COG output)")
    logger.info(f"Input Raster Paths: {len(raster_paths_str)} files")
    ...
    try:
        ...
        logger.log(logging.INFO, msg=success_payload, extra={"level": "SUCCESS"})

    except (ValueError, RuntimeError, ImportError) as e:
        logger.error(f"Job failed due to error: {e}", exc_info=True)
        error_payload = f"{JOB_ID} job run failed: {e}"
        logger.log(logging.CRITICAL, msg=error_payload, extra={"level": "ERROR"})
        exit_code = 1
    except Exception as e:
        logger.error(f"An unexpected error occurred: {e}", exc_info=True)
        error_payload = f"{JOB_ID} job run failed: An unexpected error occurred - {e}"
        logger.log(logging.CRITICAL, msg=error_payload, extra={"level": "ERROR"})
        exit_code = 1
